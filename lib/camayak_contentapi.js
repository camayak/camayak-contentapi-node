...
class CamayakContentAPI {
    ...
    generate_sig(api_key, secret) {
      ...
      let hmac = crypto.createHmac("sha1", api_key);
      hmac.update(date);
      hmac.update(secret);
    ...
    receive_webhook(req, res) {
      ...
      if (event === "validate") {
        ...
        // Create a Webhook object to handle responses
        let webhook = new WebHook(res);
        ...
        request.get(url, function(err, response, body) {
          ...
          if (event === "publish" && content.published_id) {
            ...
            return that.onUpdate(webhook, content)
          } else if (event === "publish") {
            // Callt he PUBLISH handler
            return that.onPublish(webhook, content)
          } else if (event === "retract" && content.published_id) {
            // call the RETRACT handler
            return that.onRetract(webhook, content)
          } else {
            // Error handler for unexpected situation, like a
            //  retract event, but no published_id
            return that.onError("Unexpected Error", webhook);
          }
          });
      } else {
        // No event type, malformed body, or unknown event type was sent.
        return that.onError("Unknown event type", webhook);
      }
    }
